...
export const loincSearch = async (vsUrl: string, filter: string, offset: number, count: number): Promise<{ page: string[], total: number, exhausted: boolean, fallback?: string }> => {
   const
...
export const snomedSearch = async (vsUrl: string, filter: string, offset: number, count: number): Promise<{ page: string[], total: number | undefined }> => {
   const
